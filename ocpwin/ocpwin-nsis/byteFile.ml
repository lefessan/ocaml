...
let string_of_file t =
  ...
  (match t.runtime with None -> () | Some runtime ->
    Printf.bprintf b "Runtime: %s\n" runtime);
  Buffer.add_string b "Imported units:\n";
  List.iter (fun (s, digest) ->
    Printf.bprintf b "\t%s\t%s\n" (Digest.to_hex digest) s
  ) t.imports;
  Buffer.add_string b "DLL path:\n";
  List.iter (fun s -> Printf.bprintf b "\t%s\n" s) t.dll_path;
  Buffer.add_string b "Used DLLs:\n";
  List.iter (fun s -> Printf.bprintf b "\t%s\n" s) t.dll_names;
  Buffer.add_string b "Primitives used:\n";
  Array.iter (fun s -> Printf.bprintf b "\t%s\n" s) t.primitives;
  Printf.bprintf b "Global table size: %d\n" (Array.length t.data);
  Printf.bprintf b "Debugging information: %d positions\n" (List.length t.debug);
  Printf.bprintf b "Symbols: %d\n" t.nsymbols;
  Buffer.contents b

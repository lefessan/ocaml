...
let get_output1 ?use_bash cmd =
  match get_output ?use_bash cmd with
    line :: _ -> line
  | _ ->
    Printf.eprintf "Error in get_output %S\n%!" cmd;
    exit 2
...
let new_cmdline () =
  ...
  | `MSVC | `MSVC64 | `LIGHTLD | `MINGW64R | `MINGWR -> true
...
exception CannotFindFile of string * string list
...
let find_file =
  ...
  fun fn ->
    ...
    with Not_found ->
	  ...
	  | Some fn -> fn
	  | None ->
	      raise (CannotFindFile (fn, "" :: !search_path))
        in
        if !verbose > 0 then Printf.eprintf "%S solved to %S\n%!" k r;
...
let cmd_verbose cmd = Sys.command cmd
...
let dll_exports fn = match !toolchain with
  ...
  | `GNAT | `CYGWIN | `CYGWIN64 | `MINGW | `MINGW64 | `MINGW64R | `MINGWR ->
...
let patch_output output_file =
  ...
  | Some x ->
    begin try
            try
              Stacksize.set_stack_reserve output_file x
            with e ->
              let basename = Filename.basename output_file in
              if not (String.contains basename '.') then
                let output_file = output_file ^ ".exe" in
                Stacksize.set_stack_reserve output_file x
      with e ->
        Printf.eprintf "Warning: Executable created, but stacksize could not be set\n%!"
    end
  | None -> ()
  ...
  let cmd = match !toolchain with
	...
	Printf.sprintf
	  ...
	  files
          def_file
	  extra_args
    | `MINGW | `MINGWR | `MINGW64 | `MINGW64R | `GNAT ->
        let def_file =
          if main_pgm then ""
          else
            let def_file, oc = open_temp_file "flexlink" ".def" in
...
let rec filename_concats list =
  match list with
    [] -> assert false
  | [ x ] -> x
  | x :: tail -> Filename.concat x (filename_concats tail)
...
let setup_toolchain () =
  let mingw_libs pre =
    ...
    begin match get_output (!gcc ^ " -print-sysroot") with
      line :: _ -> search_path := !search_path @ [line ^ "/mingw/lib"];
    | [] ->
      (* on Debian: *)
  ...
  let mingwr_libs pre =
    ...
    search_path := !dirs @ [
      filename_concats [prefix; pre; "lib" ];
(* Beware: we have hardcoded the version of GCC used ! *)
      filename_concats [prefix;"lib";"gcc";pre ;"4.8.1" ];
    ];
    default_libs :=
      ["-lmingw32"; "-lgcc"; "-lmoldname"; "-lmingwex"; "-lmsvcrt";
       "-luser32"; "-lkernel32"; "-ladvapi32"; "-lshell32" ];
...
(* Beware: we have hardcoded the version of GCC used ! *)
  ...
  | `MINGWR ->
      mingwr_libs Version.mingw_prefix
  | `MINGW64 ->
      mingw_libs Version.mingw64_prefix
  | `MINGW64R ->
      mingw64r_libs Version.mingw64_prefix
  | `GNAT ->
...
let all_files () =
  ...
  (* Fabrice: Instead of looking for `flexdll_*_<toolchain>.obj` only
in flexlink.exe directory, we put it on top of the lookup path. This way,
we keep the old behavior for Mingw, but for MSVC, we provide the files
for different versions inside other per-version directories.
A possible problem is that there could be a conflict, a library residing in
flexlink.exe's directory could be chosen over one specified using -I.
  *)
  (*  let f = Filename.concat flexdir in *)
  search_path := flexdir :: !search_path;
  let f s = s in
  ...
  | `MINGW64R -> "mingw64.o"
  | `GNAT    -> "gnat.o"
  | `MINGW | `MINGWR | `LIGHTLD -> "mingw.o" in
  let files = if !ocaml then
      f ("flexdll_ocaml_" ^ tc) :: files
    else files
  in
...
let main () =
  ...
  use_cygpath :=
    begin
      ...
      | _, `Yes -> true
      | _, `No -> false
      | (`GNAT|`MINGW|`MINGWR|`MINGW64|`MINGW64R|`CYGWIN|`CYGWIN64), `None ->
...
let () =
  ...
  with
    ...
    | CannotFindFile (file, path) ->
      Printf.eprintf "Cannot find file %S in:\n" file;
      List.iter (fun dir ->
        Printf.eprintf "  tried in %S\n" dir;
      ) path;

...
(* ocpwin: add `MINGWR and `MINGW64R for relocatable version *)
let toolchain : [ `MSVC | `MSVC64 | `MINGW | `MINGWR | `MINGW64 | `MINGW64R | `GNAT | `CYGWIN | `CYGWIN64 | `LIGHTLD ] ref = ref `MSVC
let save_temps = ref false
let show_exports = ref false
let show_imports = ref false
let dry_mode = ref false
let verbose = ref (try int_of_string (Sys.getenv "FLEXLINK_VERBOSE")
  with Not_found -> 0)
let dirs = ref []
let ocaml = ref false
...
let specs = [
  ...
  "-ocaml", Arg.Set ocaml, " Build an OCaml executable";
  ...
  (* Fabrice: we fixed Flexlink here (-I and -L are synomyms), because
     it processes the search path in reverse order, compared to OCaml
     !! *)
  "-I", Arg.String (fun dir -> dirs := !dirs @ [dir]),
  "<dir> Add a directory where to search for files";

  "-L", Arg.String (fun dir -> dirs := !dirs @ [dir]),
  "<dir> Add a directory where to search for files";

  "-l", Arg.String (fun s -> files := ("-l" ^ s) :: !files),
  "<lib> Library file";

  "-chain", Arg.Symbol (["msvc";"msvc64";"cygwin";"cygwin64";
                         "mingw";"mingwr";"mingw64";"mingw64r";"gnat";"ld"],
			(fun s ->
			  ...
			  | "mingw64r" -> machine := `x64; underscore := false; `MINGW64R
                          | "ld" -> `LIGHTLD
			  | _ -> assert false)),

...
method select_addressing _chunk arg = (Iindexed, arg)

method! select_operation op args =
  let norm_op =
    match op with Cadda -> Caddi | _ -> op in
  let norm_args =
...
(* We don't use anymore the mult-add instructions, as they have not
   the same precision as the decomposition (they are actually more
   precise), but we want complete compatibility with other systems.

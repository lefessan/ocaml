...
/* [caml_set_minor_heap_size_hook] is called in function
[caml_set_minor_heap_size].  It is first called with the new size of
the minor heap before any change, and then when leaving the function,
either with 0 (success) or -1 (exception) */
EXTERN_HOOK1(caml_set_minor_heap_size_hook, intnat bsz);

/* [caml_do_compaction_hook] is called in function [do_compaction] at
the beginning [position=0], between pass 3 and 4 [position=1] and at
the end [position=2]. */
EXTERN_HOOK1(caml_do_compaction_hook, intnat position);
...
/* [caml_garbage_collection_hook] is called in [caml_garbage_collection]
   (asmrun/signals_asm.c) with 0 on entry and 1 before exit. */
EXTERN_HOOK1(caml_garbage_collection_hook, intnat position);

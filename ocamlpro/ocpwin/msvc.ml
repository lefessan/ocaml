...
let break_path path = Misc.split path path_sep.[0]
  ...
  | NotTried ->
    let s =
     Printf.sprintf "%s:%s:%s"
      detect_msvc (try Sys.getenv "OCPWIN_MSVC" with _ -> "") Config.detect_msvc
    in
    let flags = Misc.split s ':' in
    let rec iter flags =
      match flags with
        [] ->
...
let () =
  ...
  Ccomp.external_command := (fun cmd ->
      Ocpstd.Sys.system cmd
    );

  let compile_file = !Ccomp.internal_compile_file in
  Ccomp.internal_compile_file := (fun name ->
      let res = compile_file name in

...
/* for "gettimeofday" */
/* Note that, under MSVC, this function is not available. However, we added
it in flexdll_ocaml.c, in ocaml/4.01.0+ocp1/cross/flexdll, so it should still
work when linked using MSVC ! */
#include <sys/types.h>
...
#ifndef OCP_SHARED_MEM_FOR_WINDOWS
/* for "waitpid" */
#include <sys/wait.h>
#endif
...
#warning "TODO: msync+munmap"
  ...
  if( ocp_memprof_shared_mem_filename != NULL ){
    CLOSE_SHARED_FD( ocp_memprof_shared_mem_fd );
...
#include <direct.h>
#include <io.h>
#define caml_mkdir(dirname, flags) _mkdir(dirname)
/* some stuff for access() */
#define caml_access _access
#define R_OK 4
#define W_OK 2
#ifdef _MSC_VER
#define X_OK 0
#endif
#else
#define caml_mkdir(dirname, flags) mkdir(dirname, flags)
#define caml_access access
#endif
...
#warning "TODO: getuid"
    ...
    ret = caml_stat(buf, &st);
    if( ret != 0 ){
      if( errno == ENOENT ){ caml_mkdir(buf, 0700); }
      ret = caml_stat(buf, &st);
    }
    ret = caml_access( buf, R_OK | W_OK | X_OK );
...
#ifdef OCP_SHARED_MEM_FOR_WINDOWS
    {
      ...
      ocp_memprof_shared_mem_fd = CreateFile(buf, GENERIC_READ | GENERIC_WRITE,
...
int32 win32_getppid_c(void)
{
  ...
  if( hProcessSnap == INVALID_HANDLE_VALUE ){
    fprintf(stderr, "win32_getppid_c: error in CreateToolhelp32Snapshot\n");
    return -1;
  }
  ...
  {
    fprintf(stderr, "win32_getppid_c: error in Process32First\n");
  ...
  fprintf(stderr, "win32_getppid_c: error pid not found\n");
  return -1;
}
#endif

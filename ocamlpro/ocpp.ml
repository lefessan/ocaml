...
type state =
  | AfterIf of bool
  | AfterElse of bool
  | InIncludedFileAt of in_channel * Lexing.lexbuf
...
let default_macros = ref
    [
      "OCAML_VERSION", [Ocpp_version.mk_string Sys.ocaml_version];
...
*)
  ...
  | EOL -> "EOL"
  | OCPP_STRUCTURE _ -> "OCPP_STRUCTURE"
  | OCPP_SIGNATURE _ -> "OCPP_SIGNATURE"
  | OCPP_PP_DIRECTIVE s -> Printf.sprintf "<<OCPP_PP_DIRECTIVE %S>>" s
  | OCPP_PP_CONTENT s -> Printf.sprintf "<<OCPP_PP_CONTENT %S>>" s
...
let string_of_directive = function
  ...
  | OCPP_BEGIN_PP -> "OCPP_BEGIN_PP"
  | OCPP_END_PP -> "OCPP_END_PP"
...
let set_ocaml_version version =
  default_macros := !default_macros @ [ "OCAML_VERSION",
                                        [Ocpp_version.mk_string version] ]

let add_macro name macro =
  default_macros := !default_macros @ [ name, macro ]
...
let parse_expr tokens lexbuf =
  ...
  P.ocpp_expr (fun lexbuf ->
      ...
      | token :: tail -> tokens := tail;
        ...
        | _ -> Ocpp_version.token_of_token token
    ) lexbuf
...
let read_token lexer lexbuf =
  ...
  | [] ->
    ...
    if debug_ocpp then begin
      Printf.eprintf "preprocess[%s]%s[%d] <- lexer: %s\n%!"
...
let rec get_token lexer lexbuf =
  let token = read_token lexer lexbuf in
  match expand_token token queued_tokens with
  | None ->
    get_token lexer lexbuf
  | Some token -> token

let bool_of_expr = function
  | String s -> failwith "bool(String)"
  | Int n -> n <> 0
  | Undefined -> failwith "bool(Undefined)"
...
let rec preprocess lexer lexbuf =
  let token = get_token lexer lexbuf in
  if debug_ocpp then begin
  ...
  match token with
  | EOL -> after_eol := true; preprocess lexer lexbuf
  ...
  | SHARP when !after_eol ->
    ...
    begin match get_token lexer lexbuf with
    | INCLUDE ->
      preprocess_directive lexer lexbuf OCPP_INCLUDE
    | LIDENT "define" ->
      preprocess_directive lexer lexbuf OCPP_DEFINE
    | LIDENT "defun" ->
      preprocess_directive lexer lexbuf  OCPP_DEFUN
    | LIDENT "undef" ->
      preprocess_directive lexer lexbuf OCPP_UNDEF
    | LIDENT "ifdef" ->
      preprocess_directive lexer lexbuf OCPP_IFDEF
    | LIDENT "ifndef" ->
      preprocess_directive lexer lexbuf OCPP_IFNDEF
    | IF ->
      preprocess_directive lexer lexbuf OCPP_IF
    | LIDENT "elif" ->
      preprocess_directive lexer lexbuf OCPP_ELIF
    | ELSE ->
      preprocess_directive lexer lexbuf OCPP_ELSE
    | LIDENT "endif" ->
      preprocess_directive lexer lexbuf OCPP_ENDIF
    | LIDENT "error" ->
      preprocess_directive lexer lexbuf OCPP_ERROR
    | LIDENT "warning" ->
      preprocess_directive lexer lexbuf OCPP_WARNING
    | LIDENT "begin_pp" ->
      preprocess_directive lexer lexbuf OCPP_BEGIN_PP
    | LIDENT "end_pp" ->
      preprocess_directive lexer lexbuf OCPP_END_PP
    | LIDENT "option" ->
      preprocess_directive lexer lexbuf OCPP_OPTION
    | token ->
      if debug_ocpp then
  ...
  | EOF ->
    begin
      match !stack with
        [] -> EOF
      | (InIncludedFileAt (ic, old_lexbuf), _) :: stack_tail ->
        after_eol := true;
        close_in ic;
        if debug_ocpp then
  ...
  | _ ->
    after_eol := false;
    if !keep then token else
      preprocess lexer lexbuf

and preprocess_directive lexer lexbuf directive =
  let rec iter lexer lexbuf tokens =
    let token =  read_token lexer lexbuf in
    match token with
    | EOF | EOL -> queued_tokens := token :: !queued_tokens; List.rev tokens
    | COMMENT _ -> iter lexer lexbuf tokens
    | _ -> iter lexer lexbuf (token :: tokens)
  in
  let args = iter lexer lexbuf [ ] in
  ...
  let do_if args =
    ...
    let rec eval_expr expr =
      ...
      | Pexp_uident _ -> Undefined
      | Pexp_ident( { txt = Lident "ocaml_version" } ) ->
        begin try
                match (StringMap.find "OCAML_VERSION" !env).macro_tokens with
                | [ (STRING _) as token ] ->
                  Version (version_of_string (Ocpp_version.get_STRING token))
                | _ -> raise Not_found
          with Not_found ->
            Printf.eprintf "Error: ocaml_version used, but OCAML_VERSION not defined, or not a string\n%!";
            exit 2
        end
      | Pexp_apply( { desc = Pexp_ident( { txt = Lident action } ) },
                    args ) ->
        begin
          match action, args with
          | "&&", [ left; right ] ->
    ...
    | Int n ->
      ...
      keep := true
    | Undefined ->
      failwith "value of #if is undefined, integer"
    | _ ->
  ...
  | OCPP_OPTION ->
    if !keep then begin
      ...
      | [ UIDENT name; STRING _ as token ] ->
        set_option name (Ocpp_version.get_STRING token)
  ...
  | OCPP_INCLUDE ->
    if !keep then begin
      ...
      match args with
        [ STRING _ as token ] ->
          preprocess_include lexer lexbuf  (Ocpp_version.get_STRING token)
      | _ ->
      ...
      match  expand_tokens args with
        [ STRING _ as token ] ->
          ...
          Format.eprintf "%aWarning: %s\n@."
            Location.print loc  (Ocpp_version.get_STRING token);
  ...
  | OCPP_BEGIN_PP ->
    if !keep then begin
      ...
      in
      match args with
      | [ STRING _ as token ] -> set_pp_state  (Ocpp_version.get_STRING token)
      | [ LIDENT name ] ->
        begin try
    ...
    preprocess Ocpp_lexer.lexer lexbuf
      ...
      if !keep then begin
        ...
        | Some ps ->
          ...
          if is_ast_file then begin
            ...
            preprocess_include lexer lexbuf tmpfile
          end
      end else
        preprocess lexer lexbuf
...
and preprocess_include lexer lexbuf  filename =
  let filename =
  ...
  stack := (InIncludedFileAt (ic, !current_lexbuf), true) :: !stack;

  if debug_ocpp then Printf.eprintf "Pushing new lexbuf on stack\n%!";
  current_lexbuf := Lexing.from_channel ic;
...
(*    Printf.eprintf "new_run with new lexbuf\n%!"; *)
  ...
  try
    let token = preprocess lexer lexbuf in
    if debug_ocpp then
  ...
  if not !registered then begin
    ...
    Ocpstd.Arg.default_arglist :=
      ("-D", Ocpstd.Arg.String (fun s ->
        add_macro s (try [Ocpp_version.mk_string (snd (Misc.cut_at s '='))]
          with Not_found -> [])),
...
(* proposition: howto merge ocpp directives with other preprocessor
   directives ?

   -ocpp-pp "preprocessor=..."
   -pp "preprocessor" <=> -ocpp-pp "default=preprocessor"

   It is always assumed that a preprocessor is called on a toplevel
   expression, and will return either a parsed tree (of type
   Parsetree.structure for
   implementations or of type Parsetree.signature for interfaces), or a string
   to be lexed.

   Between #begin_pp and #end_pp, all other directives are accepted, but
   no macro expansion is performed.

   #begin_pp prepprocessor
   ...
   #end_pp

   We need:
   1/ to accumulate chars in Buffer.t
   2/ to initialize Buffer.t with the correct number of newlines '\n' since
   the beginning of the file, or '#line "filename"' directives
...
let internal_interface_rewriters = ref []
let internal_implementation_rewriters = ref []
let add_internal_interface_rewriter name f =
  internal_interface_rewriters := (name, f) :: !internal_interface_rewriters
let add_internal_implementation_rewriter name f =
  internal_implementation_rewriters :=
    (name, f) :: !internal_implementation_rewriters
let apply_internal_interface_rewriters sourcefile intf =
  Ocputils.fold_rewriters !internal_interface_rewriters sourcefile intf
let apply_internal_implementation_rewriters sourcefile impl =
  Ocputils.fold_rewriters !internal_implementation_rewriters sourcefile impl

...
*)
  ...
  | NONREC -> "NONREC"
  | HASHOP op -> Printf.sprintf "SHARPOP(%S)" op
  | DOCSTRING docstring ->
...
let string_of_token token =
  ...
  | COLONGREATER -> ":>"
  | COMMA -> ","
  | COMMENT (comment, _pos) ->
      Printf.sprintf "(* %s *)" (String.escaped comment)
  | CONSTRAINT -> "constraint"
  | DO -> "do"
  | DONE -> "done"
  | DOT -> "."
...
let parse_expr tokens lexbuf =
  ...
  P.ocpp_expr (fun _lexbuf ->
      ...
      | token :: tail -> tokens := tail;
        match token with
        | INT (n, None) -> P.INT (int_of_string n)
        | DOT -> P.DOT
        | TRUE -> P.TRUE
        | FALSE -> P.FALSE
        | LIDENT s -> P.LIDENT s
...
let bool_of_expr = function
  | String _s -> failwith "bool(String)"
  | Int n -> n <> 0
...
let rec preprocess lexer lexbuf =
  ...
  match token with
  | EOL -> after_eol := true;
      EOL (* Change for Docstrings, was: preprocess lexer lexbuf *)
...
*)
  ...
  | HASH when !after_eol ->
    if debug_ocpp then
      Printf.eprintf "HASH maybe directive...\n%!";
...
and preprocess_directive lexer lexbuf directive =
  ...
  | OCPP_OPTION ->
    if !keep then begin
      ...
      | [ UIDENT name; INT (s, None) ] ->  set_option name s
...
let activate_ocpp = try
    ignore (Sys.getenv "OCPP_DISABLED"); false
  with Not_found -> true
  ...
  if not !registered then begin
    ...
    Clflags.arg_spec :=
      ...
      ("-ocpp-pp", Arg.String (fun s ->
       ...
       "<pp[=command]> Define an external preprocessor for Ocpp") ::
...
let _ =
  ...
  add_option "Warn" (fun _name v -> Warnings.parse_options false v);
  add_option "Warn_error" (fun _name v -> Warnings.parse_options true v);
  add_macro "OCAML_VERSION" [Ocpp_version.mk_string Sys.ocaml_version];
  add_macro "HAS_OCPP" [ INT ("1",None) ];
  register ()

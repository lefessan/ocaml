...
let mkoperator name pos =
  ...
  { desc = Pexp_ident(mkloc (Lident name) loc); loc = loc }
...
ocpp_expr:
  | LIDENT
      { mkexp(Pexp_ident (mkrhs (Lident $1) 1)) }
  | UIDENT
      { mkexp(Pexp_uident (mkrhs (Lident $1) 1)) }
  | ocpp_constant
  ...
  | LIDENT LPAREN ocpp_expr RPAREN
      { mkexp(Pexp_apply(mkexp(Pexp_ident(mkrhs (Lident $1) 1)), [ $3 ])) }
  | INT DOT ocpp_version
    {
      ...
      mkexp(Pexp_apply(mkexp(Pexp_ident(mkrhs (Lident "version") 1)),
          [ version ])) }

...
module Buf : sig
  ...
  let dump oc b =
    patch b;
    output_string oc (Bytes.to_string (Bytes.sub b.buf 0 b.pos))
  ...
  let blit_int32 s pos i =
    ...
    Bytes.set s (pos+1) (
      Char.chr ((Int32.to_int (Int32.shift_right i 8)) land 0xff) );
    Bytes.set s (pos+2) (
      Char.chr ((Int32.to_int (Int32.shift_right i 16)) land 0xff) );
    Bytes.set s (pos+3) (
      Char.chr ((Int32.to_int (Int32.shift_right i 24)) land 0xff) )
...
let output_bytes oc s = Buffer.add_string oc s
...
let extract_buf oc =
  ...
  List.iter (fun (pos, i) -> str_int32 s pos i) !patches;
  patches := [];
  Bytes.to_string s
  ...
  let put strtbl oc s =
    ...
    match s with
      | { storage = 105; extra_info = `Alias s' } when s'.sym_pos >= 0 ->
          ...
          output_bytes oc (String.sub s.auxs 4 (String.length s.auxs - 4))
      | { storage = 3; extra_info = `Section s' } when int8 s.auxs 14 = 5 (* IMAGE_COMDAT_SELECT_ASSOCIATIVE *) ->
          (* section def *)
          output_bytes oc (String.sub s.auxs 0 12);
          emit_int16 oc s'.sec_pos;
          output_bytes oc (String.sub s.auxs 14 (String.length s.auxs - 14))
...
module Section = struct
  ...
  let get filebase strtbl symtbl ic base =
    ...
    let name =
      if buf.[0] = '/'
      then strtbl (int_of_string
            (strz buf 1 ~max:7 '\000'))
...
(*        `Lazy (ic, filebase + int32_ buf 20, size) *)
  ...
  let dump x =
    Printf.printf "Section %s (0x%08lx: %s)\n"
      x.sec_name
      x.sec_opts
      (Bytes.to_string (flags x.sec_opts));
...
(*      Printf.printf "-> %s (size %i)\n" name size;  *)
    ...
    let rec read_member () =
      ...
      let size = int_of_string
          (strz (String.sub buf 48 10) 0 ' ') in

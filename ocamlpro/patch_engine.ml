...
module CmmgenPatch = ParsetreeMap.MakeMap(struct
      ...
      begin try
        ...
        with Not_found ->
          try
            ...
            | Some ({ ppat_desc = Ppat_var var } as arg_pat) ->
              {
                pat with ppat_desc = Ppat_construct(loc, Some
  ...
  let leave_expression exp =
    ...
    | Pexp_apply({ pexp_desc = Pexp_send(_, name) } as send_exp,
                 args) ->
      begin
        try
...
#if OCAML_VERSION = "4.01.0+ocp1"
    | Psig_value (loc, ({ pval_prim = prim :: _ } as val_desc) ) ->
#else
    | Psig_value ({ pval_prim = prim :: _ } as val_desc ) ->
#endif
      begin try
              ...
              if verbose then
                Printf.eprintf "Patch: prim %S add flags %s\n%!" prim
                  (String.concat " + " flags);
              let newval = { val_desc with pval_prim = val_desc.pval_prim @ flags } in
              { sg with psig_desc =
#if OCAML_VERSION = "4.01.0+ocp1"
...
#if OCAML_VERSION = "4.01.0+ocp1"
    | Pstr_primitive (loc, ({ pval_prim = prim :: _ } as val_desc) ) ->
#else
    | Pstr_primitive (({ pval_prim = prim :: _ } as val_desc) ) ->
#endif
      begin try
              let (flags, used) = StringMap.find prim !add_external_flags in
              used := true;
              ...
              { str with pstr_desc =
                  ...
                  Pstr_primitive(
#endif
                 { val_desc with pval_prim = val_desc.pval_prim @ flags }) }
        with Not_found ->
          str
      end
